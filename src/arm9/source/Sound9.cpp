...
void SndPlaySample(const u8* sampleData, u8 volume)
{
	SND_COMMAND *cmd = &sndControl->cmd[sndControl->curCmd];

	memset(cmd, 0, sizeof(SND_COMMAND));

	cmd->cmdType  = SND_CMD_PLAYSAMPLE;
	cmd->param[0] = volume;
	cmd->param32  = (u32)sampleData;
