...
static void SndCmdPlaySample(SND_COMMAND* cmd);
...
static const COMMAND_FUNC_PTR cmdFuncTable[SND_CMD_NUM] = 
{
	...
	SndCmdSetVolume,			// SND_CMD_SETVOLUME
	SndCmdPlaySample,			// SND_CMD_PLAYSAMPLE
};
...
static void SndCmdPlaySample(SND_COMMAND* cmd)
{
	u8 volume = cmd->param[0];
	u16* header = (u16*)cmd->param32;
	u8* data = (u8*)cmd->param32;

	s16 freq = header[0];
	//s16 channels = header[1];
	s16 sampleRate = header[2];
	s16 length = header[3];

	data += sizeof(s16) * 4;

	u16 format = sampleRate == 8 ? SOUND_8BIT : SOUND_16BIT;

	u8 channel;
	for (channel = sndMod.channelCount; channel < SND_MAX_CHANNELS; ++channel)
	{
		if ((SCHANNEL_CR(channel) & SCHANNEL_ENABLE) == 0) break;
	}
	if (channel == SND_MAX_CHANNELS) channel = sndMod.channelCount;

	SCHANNEL_SOURCE(channel) = (u32)data;
	SCHANNEL_REPEAT_POINT(channel) = 0;
	SCHANNEL_LENGTH(channel) = length/4;
...
static void MODHandleUpdateFlags(MOD_UPDATE_VARS *vars)
{
	...
	else
	{
		...
		SCHANNEL_CR(vars->chnIdx) = 
			SOUND_VOL(vars->sndChn->vol*2/5) | // hack to change the "mix" btwn music/sound
